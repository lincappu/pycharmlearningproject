...
def  get_crawl_interal():
    interal=random.randint(cf.get('min_crawl_intera'),cf.get('max_crawl_interal'))
...
def get_broker_and_backend():
    ...
    sentinel_args = redis_info.get('sentinel', '') # 如果不存在就是设为默认值
    db=redis_info.get('broker',5)
    if sentinel_args:
        broker_url=';'.join('sentinel://{}:@{}:{}/{}'.format(password,sentinel['host'],sentinel['port'],db)
